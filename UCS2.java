...
public class UCS2 {
    ...
    public boolean isEqual2(Node node1, Set<Node> nodes2) {
        GameBoard4 board1 = node1.state2;
        int cost1 = node1.cost;

        for (Node node2 : nodes2) {
            GameBoard4 board2 = node2.state2;
    ...
    public boolean isEqualBoard(GameBoard4 board1, GameBoard4 board2) {
    ...
    public int ucs(int[][] graph, GameBoard4 start) {
        ...
        while (!priorityQueue.isEmpty()) {
            ...
            GameBoard4 currentState = currentNode.state2;
    ...
    private void printPath(Node node) {
        List<GameBoard4> path = new ArrayList<>();
        while (node != null) {
            path.add(node.state2);
            node = node.parent;
        }
